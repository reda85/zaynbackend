...
var import_defaultTheme = __toESM(require("tailwindcss/defaultTheme"));
...
function rem(value, ptPerRem = PT_PER_REM) {
...
// src/index.ts
function themeResolver(name) {
  return import_defaultTheme.default[name] ?? {};
}
function createTw(config, options) {
  const theme = { ...import_defaultTheme.default, ...config };
  ...
  function isMultiProperty(value) {
  ...
  function parseValue(value, property, isNegative) {
    ...
    if (valueParts[0] && valueParts[0] in colors && property !== "fontWeight") {
      const color = colors[valueParts[0]];
      return {
        value: typeof color === "string" ? color : valueParts[1] && typeof valueParts[1] === "string" ? color?.[valueParts[1]] : void 0,
        type: "color",
        isCustom: false,
  ...
  function parseUtility(className) {
    ...
    const utilityParts = utilityStr ? utilityStr.slice(isNegative ? 1 : 0).split("-") : [];
  ...
  return function(input) {
    ...
    return classNames.map((className) => {
      ...
      if ("transform" in val) {
        ...
        return {
          ...acc,
          ...transform ? { transform: [acc.transform ?? "", transform].join(" ").trim() } : null,
          ...rest
        };
      }
      return { ...acc, ...val };
